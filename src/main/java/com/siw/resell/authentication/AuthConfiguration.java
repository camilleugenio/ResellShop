package com.siw.resell.authentication;

import static com.siw.resell.model.Credentials.ADMIN_ROLE;
import static com.siw.resell.model.Credentials.DEFAULT_ROLE;
...
public class AuthConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				.antMatchers(HttpMethod.GET, "/", "/index", "/home", "/login", "/login-error", "/logout", "/register",
						"/css/**", "/images/**", "/prodotto-foto/**", "/prodotto/all", "/categoria/**",
						"/sottocategoria/**", "/luogo/**")
				.permitAll().antMatchers(HttpMethod.POST, "/login", "/register").permitAll()
				/**
				 * .antMatchers(HttpMethod.GET, "/prodotto/**", "/user/**", "/categoria/**",
				 * "/sottocategoria/**", "/luogo/**", "/user/**").hasAnyAuthority(DEFAULT_ROLE,
				 * ADMIN_ROLE)
				 **/
				// DEFAULT può inserire, modificare e cancellare i propri prodotti, ADMIN lo può
				// fare anche se non è un suo prodotto
				.antMatchers(HttpMethod.POST, "/prodotto").hasAnyAuthority(DEFAULT_ROLE, ADMIN_ROLE)
				// ADMIN può modificare, cancellare ed inserire nuove categorie, sottocategorie
				// e luoghi
				.antMatchers(HttpMethod.GET, "/admin/**").hasAnyAuthority(ADMIN_ROLE)
				.antMatchers(HttpMethod.POST, "/admin/**", "/categoria", "/sottocategoria", "/luogo")
				.hasAnyAuthority(ADMIN_ROLE).anyRequest().authenticated()
	 ...
	 * used to encrypt and decrpyt the Credentials passwords.
	 */
	@Bean
